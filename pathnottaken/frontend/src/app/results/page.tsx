  ...
  useEffect(() => {
    async function loadResults() {
      try {
        ...
        // Persist AI-generated careers so the roadmap page can find them
        try {
          const aiCareers = result.recommendations.filter((c: CareerRecommendation) => c.source === 'ai' || c.id?.startsWith('ai-'));
          if (aiCareers.length > 0) {
            localStorage.setItem('pn_ai_careers', JSON.stringify(aiCareers));
          }
          ...
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-3 mb-4">
              ...
              <span className="text-lg font-semibold text-gray-700">
                Analyzing your unique skill profile...
              </span>
            </div>
            <p className="text-sm text-gray-400">Matching careers &amp; consulting AI — usually under 10 seconds</p>
            {/* animated progress bar */}
            <div className="mt-4 mx-auto max-w-xs h-1.5 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500 rounded-full animate-[progress_10s_ease-in-out_forwards]" />
            </div>
          </div>
    ...
    <section className="min-h-screen bg-[#fafbfc]">
      ...
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 relative overflow-hidden noise-bg">
        <div className="absolute inset-0">
          ...
          <div className="absolute top-1/2 left-1/2 w-40 h-40 bg-emerald-400/5 rounded-full blur-2xl animate-blob" style={{ animationDelay: '5s' }} />
        </div>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex gap-3 animate-fade-in" style={{ animationDelay: '200ms' }}>
              <div className="glass-card-dark rounded-xl px-5 py-3 text-center min-w-[80px]">
                <p className="text-2xl font-extrabold text-emerald-400 tabular-nums">{bestMatch?.matchScore || 0}%</p>
                <p className="text-[10px] text-gray-400 mt-0.5">Top Match</p>
              </div>
              <div className="glass-card-dark rounded-xl px-5 py-3 text-center min-w-[80px]">
                <p className="text-2xl font-extrabold text-white tabular-nums">{avgMatch}%</p>
                <p className="text-[10px] text-gray-400 mt-0.5">Avg Match</p>
              </div>
              <div className="glass-card-dark rounded-xl px-5 py-3 text-center min-w-[80px]">
                <p className="text-2xl font-extrabold text-teal-400 tabular-nums">{filtered.filter(r => r.growthOutlook === 'High' || r.growthOutlook === 'Very High').length}</p>
      ...
      <div className="sticky top-16 z-30 backdrop-blur-2xl bg-white/70 border-b border-gray-200/40 shadow-[0_4px_16px_rgba(0,0,0,0.03)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-4">
          <div className="flex-1 max-w-md">
            <div className="relative group">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-emerald-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white/60 backdrop-blur-sm border border-gray-200/80 rounded-xl text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-50 focus:bg-white transition-all"
              />
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex gap-8">
          ...
          <aside className="hidden lg:block w-60 flex-shrink-0">
            <div className="sticky top-32 glass-card rounded-2xl p-5 space-y-6 shadow-sm">
              ...
              <div>
                ...
                <div className="flex justify-between text-xs text-gray-400 mt-1">
                  ...
                  <span className="font-medium text-emerald-600">
                    ₱{Math.round(salaryMin / 1000)}K+/mo
                  </span>
                  <span>₱940K+</span>
                </div>
              </div>
              ...
              <div className="lg:hidden card-static p-5 mb-4 space-y-4 animate-fade-in">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="range" min={140000} max={940000} step={40000} value={salaryMin} onChange={(e) => setSalaryMin(Number(e.target.value))} className="w-full accent-emerald-600" />
                    <div className="flex justify-between text-xs text-gray-400 mt-1">
                      <span>₱140K</span>
                      <span className="font-medium text-emerald-600">₱{Math.round(salaryMin / 1000)}K+/mo</span>
                      <span>₱940K+</span>
                    </div>
                  </div>
                  <div>
                ...
                <button onClick={() => { setSalaryMin(140000); setGrowthFilter("Any"); }} className="text-sm text-gray-500 hover:text-gray-700">Reset filters</button>
              </div>
            )}
