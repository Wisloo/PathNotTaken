...
const SERVER_SYNONYMS = {
  ...
  // ─── Browse-button skill IDs (not direct canonical IDs) ───
  "graphic-design": ["design"],
  "ux-design": ["design", "ui-design"],
  mentoring: ["teaching", "empathy"],
  finance: ["accounting", "data-analysis"],
  law: ["policy", "research"],
  operations: ["project-management", "organization"],
...
- Salary ranges must be realistic PHP (Philippine Peso) MONTHLY figures typical in the Philippines (min around 20000, max around 120000)
...
Return ONLY this JSON (no other text): {"careers":[{"title":"...","category":"...","explanation":"one sentence why this fits","salaryRange":{"min":0,"max":0},"growthOutlook":"High","missingSkills":["skill1","skill2"],"matchScore":75}]}`;
  ...
  const recommendations = rawCareers.slice(0, 3).map(
    (career, index) => {
      ...
      const salaryRange = {
        min: Math.max(18000, Math.min(80000, Number(rawSalary.min) || 30000)),
        max: Math.max(25000, Math.min(150000, Number(rawSalary.max) || 65000)),
        currency: 'PHP',
      };
      // Ensure min < max
      if (salaryRange.min >= salaryRange.max) salaryRange.max = salaryRange.min + 15000;
      ...
      const salaryLabel = salaryRange.max >= 80000 ? 'competitive' : salaryRange.max >= 55000 ? 'solid' : 'growing';
